...
if(isset($_POST['send']) && isset($_FILES['file'])){
    ...
    if(in_array($extension, $extensions) && $size <= $maxSize && $error == 0){
        ...
        elseif(empty($_POST['nom']) or  empty($_POST['mail']) or empty($_POST['password']) ){
            ...
            $req=$bdd->prepare("INSERT INTO teacher(nom,mail,password,image)VALUES(:nom,:mail,:password,:image)");
            $req->execute(array(
                'nom'=>$nom,
                'mail'=> $mail,
                'password'=>$password,
            ...
            $req2=$bdd->prepare("SELECT * FROM teacher WHERE nom=? AND mail=? AND password=? AND image=?");
            $req2->execute(array($nom,$mail,$password,$file));
            if($req2->rowCount()>0){
                $_SESSION['idteacher']=$req2->fetch()['idteacher'];
                $_SESSION['nom']=$nom;
                $_SESSION['$mail']=$mail;
