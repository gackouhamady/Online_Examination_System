<?php
    ...
    if(isset($_POST['send'])){
        ...
        $req=$bdd->prepare('SELECT COUNT(mail) AS nbre_mail FROM user WHERE mail=? AND password=?');
        $req->execute(array($_POST['mail'],$_POST['password']));
        $result1=$req->fetch();
        $req1=$bdd->prepare('SELECT COUNT(password) AS nbre_pass FROM user WHERE  mail=? AND password=?');
        $req1->execute(array($_POST['mail'],$_POST['password']));
        $result2=$req1->fetch();
    if($result1['nbre_mail']>0 && $result2['nbre_pass']>0){
        $req2=$bdd->prepare("SELECT * FROM user WHERE mail=? AND password=?");
        $req2->execute(array($mail,$password));
           if($req2->rowCount()>0){
            ...
            $_SESSION['image']=$req2->fetch()['$image'];
...
<fieldset>
    ...
    <div style=" text-align:left;margin-left:50px; font-size:1.5em;"><span>Return </span><a style="font-size:1.2em;"  href="index.php"> To register</a></div>
</body>
</html>