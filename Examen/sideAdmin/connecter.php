...
if(isset($_POST['send'])){
    ...
    $password=htmlspecialchars($_POST['password']);
        ...
        $req=$bdd->prepare('SELECT COUNT(mail) AS nbre_mail FROM admin WHERE mail=? AND password=?');
        $req->execute(array($mail,$password));
        $result1=$req->fetch();
        $req1=$bdd->prepare('SELECT COUNT(password) AS nbre_pass FROM admin WHERE mail=? AND  password=?');
        $req1->execute(array($_POST['mail'],$_POST['password']));
        $result2=$req1->fetch();
    if($result1['nbre_mail']>0 && $result2['nbre_pass']>0){
        $req2=$bdd->prepare("SELECT * FROM admin WHERE mail=? AND password=?");
